/** \file leds7.h
 * \brief Заголовочный файл описания символов 7-сегментных индикаторов
 * Константы, определяющие сегментное представление разных символов
 * \par
 * \author ARV
 * \note При описании сегментов используется "прямая" логика: единичка означает свечение сегмента.
 * Для учета рода индикатора (с общим анодом или катодом) используются
 * макрос <b>common_anode</b>, который должен быть описан в проекте, если индикатор с общим анодом.
...
/// определение сегмента А индикатора. в _BV() следует указать разряд порта МК, соответствующий этому сегменту
#define _a _BV(4) //PORTD
/// определение сегмента B индикатора. в _BV() следует указать разряд порта МК, соответствующий этому сегменту
#define _b _BV(3) //PORTD
/// определение сегмента C индикатора. в _BV() следует указать разряд порта МК, соответствующий этому сегменту
#define _c _BV(2) //PORTD
/// определение сегмента D индикатора. в _BV() следует указать разряд порта МК, соответствующий этому сегменту
#define _d _BV(1) //PORTD
/// определение сегмента E индикатора. в _BV() следует указать разряд порта МК, соответствующий этому сегменту
#define _e _BV(0) //PORTD
/// определение сегмента F индикатора. в _BV() следует указать разряд порта МК, соответствующий этому сегменту
#define _f _BV(5) //PORTD
/// определение сегмента G индикатора. в _BV() следует указать разряд порта МК, соответствующий этому сегменту
#define _g _BV(6) //PORTD
/// определение сегмента H индикатора (точка). в _BV() следует указать разряд порта МК, соответствующий этому сегменту
